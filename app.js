...
const BALANCE_HISTORY = [32, 28, 40, 35, 44, 42, 47];
...
function getMember(id) {
  return MEMBERS.find(m => m.id === id) || BANK;
}
...
function renderMembersStrip() {
  ...
  document.getElementById('members-strip').innerHTML = sorted.map(m => {
    const isMe = m.id === ME;
    const ring = isMe
      ? `box-shadow:0 0 0 2.5px #0A0A0A,0 0 0 4.5px #F5C542`
      : '';
    return `
      <div class="member-card">
        <div class="member-avatar" style="background:${m.color};${ring}">
          ${m.initials}
        </div>
...
function renderFeed() {
  ...
  document.getElementById('feed-list').innerHTML = FEED.map((item, i) => {
    ...
    return `
      <div class="feed-card ${isMe ? 'highlight' : ''}" style="animation-delay:${i * 0.04}s">
        ...
        <div class="feed-body">
          <div class="feed-desc">
            <span class="type-badge ${BADGE[item.type]}">${item.type}</span>${item.desc}
          </div>
...
function renderMarketGrid() {
  document.getElementById('mkt-grid').innerHTML = Object.keys(TYPE_META).map((type, i) => {
    ...
    const count = MARKETPLACE[type].length;
    return `
...
function renderMarketItems(type) {
    ...
    if (type === 'bounty') {
      ...
      inner = `
        ...
        <div class="mkt-item-footer">
          <div class="mkt-item-by">
            ${miniAvatarDiv(poster)}<span>${poster.name}</span>
            <span style="color:var(--gray2)">· ${item.time}</span>
          </div>
      ...
      inner = `
        ...
        <div class="mkt-item-footer">
          <div class="mkt-item-by">
            ${miniAvatarDiv(fromM)}<span>${fromM.name}</span>
            <span class="vs-divider">vs</span>
            ${miniAvatarDiv(toM)}<span>${toM.name}</span>
          </div>
          <span class="status-tag status-${item.status}">${item.status}</span>
        </div>
      ...
      inner = `
        ...
        <div class="mkt-item-footer">
          <div class="mkt-item-by">
            ${miniAvatarDiv(byM)}<span>${isMe ? 'You' : byM.name}</span>
            <span style="color:var(--gray2)">· ${item.time}</span>
          </div>
          ${isMe
  ...
  el.querySelectorAll('.buy-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const item = MARKETPLACE.offering.find(o => o.id === btn.dataset.id);
      if (item) {
        const seller = getMember(item.by);
        showToast(`Purchasing from ${seller.name}… ᴄʙ soon!`);
      }
    });
  });
}
...
function renderProfile() {
  ...
  const myTxns  = FEED.filter(t => t.from === ME || t.to === ME);
  ...
  const sorted  = [...MEMBERS].sort((a, b) => b.balance - a.balance);
  const rank    = sorted.findIndex(m => m.id === ME) + 1;
  const myOff   = MARKETPLACE.offering.filter(o => o.by === ME);

  const maxBal  = Math.max(...BALANCE_HISTORY);
  const bars    = BALANCE_HISTORY.map((val, i) => {
    const pct     = Math.round((val / maxBal) * 100);
    const isToday = i === BALANCE_HISTORY.length - 1;
    return `
  ...
  const recentRows = myTxns.slice(0, 6).map(t => {
    ...
    return `
      <div class="feed-card" style="margin-bottom:8px">
        ...
        <div class="feed-amount ${cls}">${cbNum(t.amount, sign)}</div>
      </div>
    `;
  }).join('');
  ...
  document.getElementById('profile-body').innerHTML = `
    ...
    <div class="profile-stats">
      <div class="stat-item">
        <div class="stat-value green">+${earned}</div>
        <div class="stat-label">Earned</div>
      </div>
      <div class="stat-item">
        <div class="stat-value red">-${spent}</div>
    ...
    <div class="profile-section">
      ...
      ${myOff.length
        ? myOff.map(o => `
          ...
          `).join('')
        : '<div class="empty-state">No offerings yet</div>'
      }
    </div>
...
function openSendModal() {
  ...
  document.getElementById('do-send').addEventListener('click', () => {
    ...
    closeModal();
    showToast(`Sent ${amount} ᴄʙ to ${getMember(selectedId).name}! 💸`);
  });
...
function openCreateModal(preType = null) {
  ...
  document.getElementById('do-create').addEventListener('click', () => {
    ...
    closeModal();
    showToast(`${TYPE_META[selectedType].emoji} Posted and going live!`);
  });
